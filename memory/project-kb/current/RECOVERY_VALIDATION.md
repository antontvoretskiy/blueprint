...
| Current state is recoverable | `08_CURRENT_STATE.md` explains v0.11.1 and the v0.12.0 release target | PASS |
...
Repository: antontvoretskiy/blueprint
Public branch model: main only
Current recovery point: v0.11.1 released on main; v0.12.0 ZIP context export target selected
Included state: v0.11.1 public framework bundle with Markdown/JSONL context export commands and stronger release-state checks
...
`Repository-owned recovery files describe the released v0.11.1 state, the v0.12.0 release target, and the main-only public branch model without requiring previous chat history.`
